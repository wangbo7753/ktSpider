 ...
 */
public class VADSPipeline extends KTPipeline {

    private static final Logger LOG = LoggerFactory.getLogger(VADSPipeline.class);

    private DBLink dbLink;

    private static final String DOWN = "下行";

    public VADSPipeline(DBLink dbLink) {
        super();
        this.dbLink = dbLink;
    }
    ...
    public void process(ResultItems resultItems, Task task) {
        if (dbLink != null) {
            List<VADSBean> vaDataList = (List<VADSBean>) resultItems.get("VADataList");
            if (CollectionUtils.isNotEmpty(vaDataList)) {
                ...
                String sql = "INSERT INTO TRAIN (ID, PASS_TIME, DEV_ID, SITE_NAME, TRAIN_ID, DEV_TYPE, BUREAU_ID, SPEED, "
                        + "VEH_NUMBER, TRAIN_TYPE, VEH_DIRECT, C1, LINE_NAME)"
                        + "VALUES (SEQ_TRAIN.nextval, to_date(?, 'yyyy-mm-dd hh24:mi:ss'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                try {
                    conn.setAutoCommit(false);
                    ps = conn.prepareStatement(sql);
                    for (VADSBean bean: vaDataList) {
                        ps.setString(1, bean.getPassTime());
                        ps.setInt(2, queryDevId(bean.getStationName(), "A", conn));
                        ps.setString(3, bean.getStationName());
                        ps.setString(4, bean.getTrainId());
                        ps.setString(5, "A");
                        ps.setInt(6, 7);
                        ps.setInt(7, Integer.parseInt(bean.getSpeed()));
                        ...
                        ps.setInt(11, Integer.parseInt(bean.getAxleCount()));
                        ps.setString(12, bean.getLineType());
                        ps.addBatch();
                    }
                    ps.executeBatch();
                    conn.commit();
